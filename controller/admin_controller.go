 ...
 * 请求url：admin/singout
 */
func (this *AdminController) GetSignout() mvc.Result {
	// 删除session, 下次需要从新登录
 ...
 * 接口：/admin/login
 */
func (this *AdminController) PostLogin(context iris.Context) mvc.Result {
	iris.New().Logger().Info(" admin login ")
	var adminLogin AdminLogin
	this.Ctx.ReadJSON(&adminLogin)
	//数据参数检验
	...
	admin, exist := this.Service.GetByAdminNameAndPassword(adminLogin.UserName, adminLogin.Password)
	if !exist {
		return mvc.Response{
			Object: map[string]interface{}{
				"status":  "0",
	...
	this.Session.Set(ADMIN, userByte)
	return mvc.Response{
		Object: map[string]interface{}{
			"status":  "1",
			"success": "登录成功",
